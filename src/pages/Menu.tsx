...
const Menu = () => {
  ...
  useEffect(() => {
    if (!esSala || !restaurante?.id || !restaurante.username || !tokenTracking) return
    registrarEventoTrackingUnaVez(restaurante.id, restaurante.username, 'session_start', `sala-${tokenTracking}`)
  }, [esSala, restaurante?.id, restaurante?.username, tokenTracking])

  useEffect(() => {
    if (!esSala || !drawerOpen || !selectedProduct?.id || !restaurante?.id || !restaurante.username) return
    registrarEventoTrackingUnaVez(restaurante.id, restaurante.username, 'product_view', `sala-${tokenTracking ?? 'sin-token'}-producto-${selectedProduct.id}`, {
      productoId: selectedProduct.id, nombreProducto: selectedProduct.nombre, valor: selectedProduct.precio,
    })
  }, [drawerOpen, esSala, restaurante?.id, restaurante?.username, selectedProduct?.id, selectedProduct?.precio, tokenTracking])
  ...
  const agregarAlPedido = (producto: typeof productos[0] | any, cantidad: number = 1, ingredientesExcluidos?: number[], agregados?: any[], varianteSeleccionada?: any, varianteSecundariaSeleccionada?: any) => {
    ...
    if (esSala && restaurante?.id && restaurante.username) registrarEventoTracking(restaurante.id, restaurante.username, 'add_to_cart', {
      productoId: producto.id, nombreProducto: producto.nombre, cantidad, valor: (Number(precioUnitario) * cantidad).toFixed(2),
    })
    sendMessage({
      type: 'AGREGAR_ITEM',
      payload: {
        ...
        if (data.success && data.order) {
          if (checkoutDeliveryData?.trackingClienteId === clienteId && restaurante?.id && restaurante.username) {
            registrarEventoTrackingUnaVez(restaurante.id, restaurante.username, 'purchase', `pedido-${data.order.pedidoId}`, {
              pedidoUnificadoId: data.order.pedidoId, valor: Number(data.order.total || 0), items: data.order.items,
                ...
                {!restaurante?.soloCartaDigital ? (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (esSala) {
                        if (restaurante?.id && restaurante.username) registrarEventoTracking(restaurante.id, restaurante.username, 'checkout_start', {
                          valor: totalPedido, metadata: { cantidadItems: todosLosItems.length, grupal: true },
                        })
